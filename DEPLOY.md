...
1) Cíl: po push do větve `main` se repozitář nasadí na server a soubory se zkopírují do adresáře, který hostí web `backup.eshopion.eu`.
...
- `DEPLOY_HOST` — hostname nebo IP serveru (např. shell.r2.websupport.sk nebo backup.eshopion.eu)
...
- `DEPLOY_PATH` — absolutní cesta na serveru, kam se kopírují soubory (např. `/var/www/backup.eshopion.eu` nebo `/data/7/3/.../backup`)
...
ssh-copy-id -i ~/.ssh/deploy_keys/github_deploy.pub -p 22 uid1082847@backup.eshopion.eu
```
